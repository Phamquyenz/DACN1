...
def download_diagram_plantuml(puml_text, filename):
    print("Generating", filename)
    pl = PlantUML(url='http://www.plantuml.com/plantuml/png/')
    try:
        pl.processes(puml_text, outfile=filename)
        time.sleep(2)  # Give the server a breather
    except Exception as e:
        print("Failed download:", e)
        # Fallback to local dummy if failed, but we really want it to succeed
        pass
...
AD --> UC11
@enduml"""
    with open('uc_temp.puml', 'w', encoding='utf-8') as f:
        f.write(puml_usecase)
    download_diagram_plantuml('uc_temp.puml', "uc_diag.png")
...
stop
@enduml"""
    with open('act_temp.puml', 'w', encoding='utf-8') as f:
        f.write(puml_activity)
    download_diagram_plantuml('act_temp.puml', "act_diag.png")
...
@enduml"""
    with open('seq_temp.puml', 'w', encoding='utf-8') as f:
        f.write(puml_sequence)
    download_diagram_plantuml('seq_temp.puml', "seq_diag.png")
...
V "1" -- "0..*" O : affects >
@enduml"""
    with open('erd_temp.puml', 'w', encoding='utf-8') as f:
        f.write(puml_erd)
    download_diagram_plantuml('erd_temp.puml', "erd_diag.png")
    ...
    doc.add_paragraph('Bảng 1: Bảng products')
    create_table(doc, ['Trường', 'Kiểu dữ liệu', 'Ý nghĩa'], [
        ['id', 'BigInt', 'Khóa chính'],
        ['name', 'Varchar', 'Tên văn phòng phẩm'],
        ['sale_price', 'Decimal', 'Khuyến mãi'],
        ['stock', 'Int', 'Lượng tồn kho thực tế']
    ])

    doc.add_paragraph('\nBảng 2: Bảng vouchers')
    create_table(doc, ['Trường', 'Kiểu', 'Ý nghĩa'], [
        ['code', 'Varchar', 'Mã người dùng (Ví dụ: SALE50)'],
        ['discount_amount', 'Decimal', 'Mức giảm']
    ])

    doc.add_paragraph('\nBảng 3: Bảng order_items (Chi tiết)')
    create_table(doc, ['Trường', 'Kiểu', 'Ý nghĩa'], [
        ['order_id', 'BigInt', 'Liên kết Bảng Orders'],
        ['product_id', 'BigInt', 'Liên kết Bảng Products'],
        ['quantity', 'Int', 'Số lượng bán']
    ])

    doc.add_page_break()
    doc.add_heading('CHƯƠNG V: KẾT LUẬN', 1)
    doc.add_paragraph('Hệ thống đạt mục tiêu thiết lập thương hiệu trực tuyến "The Stationery Muse". Các điểm phức tạp như thuật toán trừ Tồn kho khi tạo Đơn hàng, Voucher hoạt động đúng. Mô hình UML đã phác hoạ rõ kiến trúc Laravel.')
